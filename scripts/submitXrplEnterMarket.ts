/**
 * Sends an ENTER_MARKET intent from XRPL Ledger to XRPL EVM via Axelar GMP (call_contract).
 *
 * The intent instructs the adapter to call comptroller.enterMarkets([market]) via the user
 * proxy, enabling the supplied cToken balance to count as collateral for borrowing.
 *
 * Required env vars:
...
async function main() {
  ...
  const intentId = ethers.keccak256(
    ethers.toUtf8Bytes(`xrpl-enter-market:${xrplWallet.address}:${nonce}:${Date.now()}`)
  );
