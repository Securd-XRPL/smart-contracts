/**
 * Sends a SUPPLY (DEPOSIT) intent for STST from XRPL Ledger to XRPL EVM using
 * the two-transaction "Add Gas" flow instead of in-kind gas deduction.
 *
 * Why this script exists (separate from submitXrplStsTSupply.ts):
 ...
 *   Amount  = supply amount only (no gas added on top)
 ...
 *
 * Intent envelope amount = supply portion only (unaffected by the gas split).
 *
 ...
 *
 * Run:
 *   ts-node scripts/submitXrplStsTSupplyAddGas.ts
 */
...
async function main() {
  ...
  const intentId = ethers.keccak256(
    ethers.toUtf8Bytes(`xrpl-stst-supply-addgas:${xrplWallet.address}:${nonce}:${Date.now()}`)
  );
