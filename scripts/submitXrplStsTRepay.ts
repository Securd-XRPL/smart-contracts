/**
 * Sends a REPAY intent for STST from XRPL Ledger to XRPL EVM.
 *
 * STST is an IOU token on XRPL. Gas is taken from the token itself (same as
 * XRP takes gas from drops). Total IOU sent = repayAmount + gasFeeToken.
 * gas_fee_amount memo = gasFeeToken (in IOU value string, e.g. "2").
 * Intent envelope amount = repay portion only.
 *
...
async function main() {
  ...
  // Total IOU sent = repay + gas (gas taken from the IOU token itself)
  ...
  const intentId = ethers.keccak256(
    ethers.toUtf8Bytes(`xrpl-stst-repay:${xrplWallet.address}:${nonce}:${Date.now()}`)
  );
  ...
  // Total IOU sent = repayAmount + gasFeeToken.
