/**
 * Sends a SUPPLY (DEPOSIT) intent for STST from XRPL Ledger to XRPL EVM.
 *
 * STST is an IOU token on XRPL. Gas is taken from the token itself (same as
 * XRP takes gas from drops). Total IOU sent = supplyAmount + gasFeeToken.
 * gas_fee_amount memo = gasFeeToken (in IOU value string, e.g. "2").
 * Intent envelope amount = supply portion only.
 *
 * Amount scaling: XRPL IOU value "10" → EVM amount = parseEther("10") = 10e18
 * (STST has 18 decimals on XRPL EVM; no drops-to-wei scaling needed).
 *
 * Required env vars:
 ...
 *   XRPL_STST_SUPPLY_AMOUNT          Amount to supply as human-readable string (e.g. "10")
 *
 * Optional:
 ...
 *   XRPL_STST_GAS_FEE                Gas fee in STST IOU units (default: "2")
 *   XRPL_DEPOSIT_INTENT_DESTINATION  Destination bytes in envelope (default: 0x)
...
async function main() {
  ...
  const supplyAmt   = requiredEnv("XRPL_STST_SUPPLY_AMOUNT");
  ...
  // STST hex currency code on XRPL: "STST" right-padded to 20 bytes
  ...
  const gasFeeToken = optionalEnv("XRPL_STST_GAS_FEE", "2");   // gas taken in STST
  const confirmSend = process.env.XRPL_CONFIRM_SEND === "true";

  // STST has 18 decimals on XRPL EVM. XRPL IOU value "10" = 10 STST = 10e18 wei.
  const supplyAmountEVM = ethers.parseEther(supplyAmt);
  // Total IOU sent = supply + gas (same pattern as XRP drops = deposit + gasFee)
  ...
  const intentId = ethers.keccak256(
    ethers.toUtf8Bytes(`xrpl-stst-supply:${xrplWallet.address}:${nonce}:${Date.now()}`)
  );
  ...
  const envelope = {
    ...
    nonce,
    deadline: BigInt(0),
    destinationAddress: optionalEnv("XRPL_DEPOSIT_INTENT_DESTINATION", "0x"),
    version: 1
  };
  ...
  // Gas is taken from the IOU token itself (same principle as XRP drops).
  // Total IOU sent = supplyAmount + gasFeeToken.
  // gas_fee_amount memo = gasFeeToken (in IOU value units, not drops).
